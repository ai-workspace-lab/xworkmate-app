...
| 候选 | 是否属于发布连接器 | 理由 |
|---|---|---|
| **邮箱** | ✅ 是（已确认纳入） | 直接操作发送与回复：一次请求一个结果，标准单向投递。 |
| **在线 Office / 文档** | ✅ 是（已确认纳入） | 直接输出成文档，对接 Google Docs 一类：单向写入，与 GitHub 发布同构。 |
| **电商平台** | ❌ 不是（已确认排除） | 见下。走 Gateway 技能包路线。 |

> 2026-08-03 决策：本轮发布连接器只扩两类——**邮箱**与**在线 Office**。电商能力改走插件目录的电商分组，见
> `docs/plans/2026-08-03-ecommerce-plugin-group.md`。
...
需求写的是「直接操作发送**与回复**」。发送只需要 SMTP，**回复需要先读到那封原始邮件**（拿到
`Message-ID` 与 `References` 才能让邮件客户端正确串进同一个会话线程），这就需要 IMAP 读取权限。

这一步越过了「发布连接器只写不读」的边界：

- 纯发送：连接器只有 `publish()`，没有任何读能力，凭据泄露的影响面是「能以你的名义发信」。
- 带回复：连接器需要 `list()` / `fetch()`，凭据泄露的影响面变成「能读你全部邮件」。

**建议拆成两个连接器**：`smtp-email`（P2，只发）与 `imap-email-reply`（延后，可读可回）。后者要单独过一次
安全评审，并在设置卡片上明示「此连接器可读取你的邮箱」。不要为了省一张卡片把读权限混进发送连接器。
...
> 需求点名了 Google Docs。它在上表里是 🔴：OAuth + 应用审核 + Markdown→Docs 格式转换，三样都要。
> 若必须优先 Google Docs，则 §4 的 OAuth 决策就从「P4 再说」变成**本轮第一优先**，整条路线要重排。
> 折中路径：先用 Drive 的 `files.create` 直接上传 Markdown 并让 Drive 自动转换为 Google Doc，
> 跳过 Docs API 的结构化 batchUpdate——能省掉格式转换这一项，但 OAuth 仍然绕不过。
...
| **待评审** | IMAP 邮件回复连接器（§3.1）| 读权限单独过安全评审，与发送连接器分开 |
| **不排期** | 电商平台 | 按 §1 的判定，走插件目录的电商分组，不进发布连接器 |
