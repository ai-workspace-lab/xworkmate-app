...
| `BRIDGE_AUTH_TOKEN` | `uTvryFvAbz6M5sRtmTaSTQY6otLZ95hneBsWqXu+35I=` |
...
|----------|----|
| `BRIDGE_SERVER_URL` | `https://xworkmate-bridge.svc.plus` |
| `BRIDGE_REVIEW_AUTH_TOKEN` | `review-beta-gYXlsS5no7OArDg5V-pTIusZme1R9iYPZ8Fo1fF4Uro` |
...
- 操作步骤
  ...
  4. 使用 `review@svc.plus` / `Review123!` 重新登录
  5. 观察同步状态与本地配置状态
- 期望结果
  - 退出后不会继续显示已登录状态
  - 重新登录成功
...
- 操作步骤
  ...
  4. 在 `鉴权令牌 (TOKEN)` 输入 `uTvryFvAbz6M5sRtmTaSTQY6otLZ95hneBsWqXu+35I=`
  5. 点击 `保存配置`
  6. 重新进入设置页确认配置仍然存在
...
- 操作步骤
  ...
  3. 在 `鉴权令牌 (TOKEN)` 输入 `review-beta-gYXlsS5no7OArDg5V-pTIusZme1R9iYPZ8Fo1fF4Uro`
  4. 点击 `保存配置`
  5. 重新进入设置页确认配置稳定
  6. 发起一次 AI 智能体工作空间任务
- 期望结果
...
- 操作步骤
  ...
  3. 在 `鉴权令牌 (TOKEN)` 输入 `uTvryFvAbz6M5sRtmTaSTQY6otLZ95hneBsWqXu+35I=`
  4. 点击 `保存配置`
  5. 发起一次 AI 智能体工作空间任务
